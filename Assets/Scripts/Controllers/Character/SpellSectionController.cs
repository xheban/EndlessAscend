...
public sealed class SpellSectionController
{
    ...
    // For now: only 4 slots unlocked at start
    ...
    public void Bind(VisualElement spellsPanel, ScreenSwapper swapper)
    {
        ...
        _active = new ActiveSpellsSectionController("Icons/27_Skull");
    ...
    public void OnTabShown()
    {
        ...
        void OnGeom(GeometryChangedEvent _)
        {
            ...
            // These are fine, but now they happen AFTER layout exists
    ...
    private void OnDroppedToActiveSlot(int targetSlotIndex)
    {
        ...
        // What is currently in the target slot?
        var targetEntry = book.GetSpellInSlot(targetSlotIndex); // may be null
        var targetSpellId = targetEntry != null ? targetEntry.spellId : null;

        // Dropped onto same slot when dragging from active -> do nothing
        ...
        {
            // active -> active : move or swap
            var sourceSlot = _dragSourceSlotIndex;

            if (string.IsNullOrWhiteSpace(targetSpellId))
            {
                // Move (target empty): just set dragged spell into target slot.
                book.SetActive(_dragSpellId, targetSlotIndex);
                // (Assumes SetActive updates activeSlotIndex and the old slot becomes empty)
            }
            else
            {
                // Swap:
                // 1) put target spell into source slot
                // 2) put dragged spell into target slot
        ...
        {
            // list -> active : replace if occupied
            if (!string.IsNullOrWhiteSpace(targetSpellId))
    ...
    {
        // Your dashboard may or may not have this panel.
    ...
    private void RefreshAll()
    {
        ...
        // Don't rebuild when the whole panel is hidden
    ...
    private ChooseSpellSlotOverlayContext BuildChooseSpellSlotContext(string spellId)
    {
        ...
        for (int slot = 0; slot < _unlockedActiveSlots; slot++)
        {
            ...
            foreach (var kvp in book.Entries) // kvp.Key = spellId, kvp.Value = PlayerSpellEntry
            {
